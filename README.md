...
| `sonar-project.properties` | SonarQube scan scope and report paths |
| `scripts/sonar-scan.sh` | Pytest + coverage, then SonarQube upload |
...
Configuration lives in [`sonar-project.properties`](sonar-project.properties) at the repo root. Use [`scripts/sonar-scan.sh`](scripts/sonar-scan.sh) to run backend tests with coverage, then upload results to a SonarQube server.

### What is analyzed

| Scope | Path | Notes |
|-------|------|-------|
| **Production source** | `frontend/src/**`, `backend/app/**` | TypeScript/TSX and Python application code |
| **Tests** | `backend/tests/**`, `frontend/src/**/*.test.ts(x)` | Marked as test code (not counted as production source) |

**Excluded from production source analysis** (`sonar.exclusions`):

- `node_modules/`, `dist/`, `alembic/`, `seeding/`
- All test files (`*.test.ts`, `*.test.tsx`, `tests/`)

**Not scanned at all** (outside `sonar.sources`):

- `loadtest/`, `docs/`, `scripts/` (except as scan helpers), Docker/infra files
- Frontend root config (`vite.config.ts`, `package.json`, etc.)

### Coverage

- **Backend only** — `sonar.coverage.exclusions=frontend/**`
- Frontend is still analyzed for bugs and code smells, but does **not** affect the coverage percentage
- Coverage report: `backend/coverage.xml` (generated by pytest; gitignored)
- Test execution report: `backend/junit.xml` (gitignored)

### Prerequisites

| Tool | Purpose |
|------|---------|
| **Docker** | `sonarsource/sonar-scanner-cli` image (used by the script) |
| **SonarQube server** | Local instance (e.g. `sonarqube:community` on port 9000) or course/shared server |
| **Backend venv** | Same as Path B — `backend/.venv` with `requirements-test.txt` |

### Run a scan (recommended)
...
# Default server URL for Docker scanner → host machine SonarQube
export SONAR_HOST_URL="http://host.docker.internal:9000"   # macOS/Windows Docker
# Linux (scanner in Docker, SonarQube on host): use http://172.17.0.1:9000 or host network
...
1. Runs `pytest backend/tests` with coverage → `backend/coverage.xml`, `backend/junit.xml`
2. Rewrites report paths for the monorepo layout (`scripts/fix-coverage-for-sonar.py`, `scripts/fix-junit-for-sonar.py`)
3. Runs `sonarsource/sonar-scanner-cli` with `sonar-project.properties`

Open the dashboard:

```text
http://localhost:9000/dashboard?id=cnad-safety-response
```

(`sonar.projectKey` is `cnad-safety-response`.)

### Manual steps (if not using the script)

```bash
# From repo root — backend venv required
...
backend/.venv/bin/python scripts/fix-coverage-for-sonar.py backend/coverage.xml
backend/.venv/bin/python scripts/fix-junit-for-sonar.py backend/junit.xml

docker run --rm \
  -e SONAR_HOST_URL="${SONAR_HOST_URL:-http://host.docker.internal:9000}" \
  -e SONAR_TOKEN="${SONAR_TOKEN:?Set SONAR_TOKEN}" \
  -v "$(pwd):/usr/src" -w /usr/src \
  sonarsource/sonar-scanner-cli
```

Create the venv first if needed:

```bash
cd backend && python3 -m venv .venv && .venv/bin/pip install -r requirements-test.txt
```

### Reproducing results (for reviewers / TAs)

To get comparable SonarQube results on a fresh clone:

1. Check out the branch or commit under review.
2. Create `backend/.venv` and install `requirements-test.txt`.
3. Start SonarQube (or connect to the course SonarQube instance).
4. Create or import project key **`cnad-safety-response`** (or keep the key in `sonar-project.properties`).
5. Align **Quality Gate** with course requirements (this repo targets **0 new issues**, **≥ 80% new coverage** on backend, **≤ 3% duplication** on new code).
6. Set `SONAR_TOKEN` and run **`./scripts/sonar-scan.sh`** — do **not** run the scanner alone; `coverage.xml` and `junit.xml` are gitignored and must be generated locally.
7. Open the project dashboard and check the **New Code** tab (baseline depends on SonarQube “New Code” settings, e.g. previous version).

| Setting | Value in this repo |
|---------|-------------------|
| Project key | `cnad-safety-response` |
| Sources | `frontend/src`, `backend/app` |
| Coverage | Backend only (`backend/coverage.xml`) |
| Host URL (Docker scanner) | `http://host.docker.internal:9000` (override with `SONAR_HOST_URL`) |
